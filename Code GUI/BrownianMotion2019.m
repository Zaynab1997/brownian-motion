...
k=R*temp/(6*pi*Na*visc*r)*10^17;
 ...
 for s=1:1:5
   ...
   pause(h);
     ...
     myworld.big.translation=[etat0(1,1)+grad(1,1)*sigma*sqrt(h)*k/50 etat0(2,1)+sigma*sqrt(h)*k*grad(1,2)/50 etat0(3,1)+sigma*sqrt(h)*k*grad(1,3)/50]
     myworld.b1.translation=[etat0(1,2)+grad(1,2)*alpha/50 etat0(2,2)+grad(2,2)*alpha/50 etat0(3,2)+grad(3,2)*alpha/50]
     myworld.b2.translation=[etat0(1,3)+grad(1,3)*alpha/50 etat0(2,3)+grad(2,3)*alpha/50 etat0(3,3)+grad(3,3)*alpha/50]
     myworld.b3.translation=[etat0(1,4)+grad(1,4)*alpha/50 etat0(2,4)+grad(2,4)*alpha/50 etat0(3,4)+grad(3,4)*alpha/50]
     myworld.b4.translation=[etat0(1,5)+grad(1,5)*alpha/50 etat0(2,5)+grad(2,5)*alpha/50 etat0(3,5)+grad(3,5)*alpha/50]
     myworld.b5.translation=[etat0(1,6)+grad(1,6)*alpha/50 etat0(2,6)+grad(2,6)*alpha/50 etat0(3,6)+grad(3,6)*alpha/50]
     myworld.b6.translation=[etat0(1,7)+grad(1,7)*alpha/50 etat0(2,7)+grad(2,7)*alpha/50 etat0(3,7)+grad(3,7)*alpha/50]
     etat0=etat1; 
 end
